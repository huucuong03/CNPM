...
<head>
    <title>Danh sách chip | Quản trị Admin</title>
    <meta charset="utf-8">
                ...
                <div class="tile-body">
                    ...
                    <div class="row element-button">
                        <div class="col-sm-2">
                            ...
                            <a class="btn btn-add btn-sm" href="/admin/${nhanVien.maNhanVien}/add/quan-li-chip"
                    ...
                    <div id="divPrint">
                        <table class="table table-hover table-bordered js-copytextarea" cellpadding="0" cellspacing="0"
                            ...
                            <tbody>
                            <c:forEach items="${chipList}" var="c">
                                <tr>
                                    <td width="10"><input type="checkbox" name="check1" value="1"></td>
                                    <td>${c.maChip}</td>
                                    <td>${c.tenChip}</td>
                                    <td><span
                                            class='${c.trangThai == 1 ? 'badge bg-success' : ' badge bg-danger'}'>${c.trangThai == 1 ? "Hoạt động" : "Ngừng hoạt động" }</span>
                                    </td>
                                    <td>
                                        <button class="btn btn-primary btn-sm trash" type="button" title="Xóa"
                                                onclick="return deleteRow(${c.maChip})"><i
                                                class="fas fa-trash-alt"></i>
                                        </button>
                                        <button class="btn btn-primary btn-sm edit" type="button" title="Sửa"
                                                data-toggle="modal" data-target="#ModalChip"
                                                onclick="editChipModal(this)">
...
MODAL
-->
<div class="modal fade" id="ModalChip" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static"
     data-keyboard="false">
                ...
                <form class="row" id="form-update">
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label class="control-label">Mã Chip</label>
                            <input class="form-control" type="text" required id="maChip" value="maChip"
                                   name="maChip" readonly>
                        </div>
                        <div class="form-group col-md-6">
                            <label class="control-label">Tên dung lượng</label>
                            <input class="form-control" id="tenChip" name="tenChip" type="text" value="tenChip">
                        </div>
                        <div class="form-group col-md-6">
...
<script>
    ...
    function deleteRow(maChip) {
        swal({
            title: "Cảnh báo",
            text: "Bạn có chắc chắn là muốn xóa Chip này?",
            buttons: ["Hủy bỏ", "Đồng ý"],
        }).then((willDelete) => {
            if (willDelete) {
                fetch('/chip/sua/' + maChip, {
                    method: 'PUT'
                })
                    .then(response => {
                        if (response.ok) {
                            ...
                            setTimeout(() => {
                                window.location.href = '/admin/' + ${nhanVien.maNhanVien} +'/quan-li-chip';
                            }, 2000);
                        } else {
    ...
    function updateConfirm() {
        ...
        var maChip = document.getElementById("maChip").value;
        var tenChip = document.getElementById("tenChip").value;
        var trangThai = document.getElementById("trangThai").value;

        // Check if required fields are filled
        if (!maChip || !tenChip || !trangThai) {
    ...
    function editChipModal(button) {
        var row = button.closest("tr");
        var maChip = row.cells[1].innerText;
        var tenChip = row.cells[2].innerText;
        var trangThai = row.cells[3].innerText;

        document.getElementById("maChip").value = maChip;
        document.getElementById("tenChip").value = tenChip;

        var selectElementNSX = document.getElementById("trangThai");
        ...
        // Mở modal
        $('#ModalChip').modal('show');
    }

    function saveChip() {
        let maChip = document.getElementById("maChip").value;
        let tenChip = document.getElementById("tenChip").value;
        let Chip = {
            maChip: maChip,
            tenChip: tenChip,
            trangThai: 1
        ...
        }).then((res) => window.location.href = '/admin/quan-li-chip');
    }
