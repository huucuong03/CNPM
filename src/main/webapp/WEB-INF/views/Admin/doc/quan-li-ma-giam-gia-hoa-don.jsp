...
<main class="app-content">
    <div class="app-title">
        <ul class="app-breadcrumb breadcrumb side">
            <li class="breadcrumb-item active"><a href="#"><b>Danh sách mã giảm giá hóa đơn</b></a></li>
        </ul>
        <div id="clock"></div>
    </div>
    <div class="row">
            ...
            <div class="tile">
                <div class="tile-body">
                    ...
                    <table class="table table-hover table-bordered" id="sampleTable">
                        ...
                        <tbody>
                        <c:forEach var="gghd" items="${gghd}">
                            <tr>
                                <td style="display: none">${gghd.maVoucher}</td>
                                <td>${gghd.ten}</td>
                                <td>${gghd.phanTramVoucher}</td>
                                <td>${gghd.soLuongToiThieu}</td>
                                <td>${gghd.donGiaToiThieu}</td>
                                <td>${gghd.ngayBatDau}</td>
                                <td>${gghd.ngayKetThuc}</td>
                                <td><span class='${gghd.statusV == 1 ? 'badge bg-success' : ' badge bg-danger'}'>${gghd.statusV == 1 ? "Hoạt động" : "Ngừng hoạt động" }</span></td>
                                <td>
<%--                                    <button class="btn btn-primary btn-sm trash" type="button" title="Xóa"--%>
<%--                                            onclick="return deleteRow(${gghd.maVoucher})"><i--%>
                ...
                <form class="row" id="form-update" >
                    <div class="row">
                        <div class="form-group col-md-6" style="display: none">
                            ...
                            <input class="form-control" type="text" required id="maVoucher" value="maVoucher"
                                   name="maGiamGia">
                        </div>
                        <div class="form-group col-md-6">
                            <label class="control-label">Tên </label>
                            <input class="form-control" type="text" required id="ten" value="ten"
                                   name="tenGiamGia">
                        </div>
                        <div class="form-group  col-md-6">
                            <label class="control-label">Phần trăm Sale</label>
                            <input class="form-control" type="number" required id="phanTramVoucher" value="phanTramVoucher"
                                   name="phamTramSale">
                        </div>
                        <div class="form-group  col-md-6">
                            <label class="control-label">Số lượng mua tối thiểu</label>
                            <input class="form-control" type="number" required id="soLuongMuaTT" value="soLuongMuaTT"
                                   name="soLuongMuaTT">
                        </div>
                        <div class="form-group  col-md-6">
                            <label class="control-label">Đơn giá mua tối thiểu</label>
                            <input class="form-control" type="number" required id="donGiaMuaTT" value="donGiaMuaTT"
                                   name="donGiaMuaTT">
                        </div>
                        <div class="form-group  col-md-6">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control" type="date" required id="ngayKetThuc" value="ngayKetThuc" name="ngayKetThuc">
                        </div>
                        <div class="form-group col-md-6">
...
<script>
    function editProductModal(button) {
        ...
        var maVoucher = row.cells[0].textContent;
        var ten = row.cells[1].textContent;
        var phanTramVoucher = row.cells[2].textContent;
        var soLuongMuaTT = row.cells[3].textContent;
        var donGiaMuaTT = row.cells[4].textContent;
        var ngayBatDau = row.cells[5].textContent;
        var ngayKetThuc = row.cells[6].textContent;
        var trangThai = row.cells[7].textContent;

        document.getElementById("maVoucher").value = maVoucher;
        document.getElementById("ten").value = ten;
        document.getElementById("phanTramVoucher").value = phanTramVoucher;
        document.getElementById("ngayBatDau").value = ngayBatDau;
        document.getElementById("ngayKetThuc").value = ngayKetThuc;
        document.getElementById("soLuongMuaTT").value = soLuongMuaTT;
        document.getElementById("donGiaMuaTT").value = donGiaMuaTT;
    ...
    function updateConfirm() {
        ...
        var phanTramVoucher = document.getElementById("phanTramVoucher").value;
        ...
        if (!ten.trim() || !phanTramVoucher || !soLuongMuaTT || !donGiaMuaTT || !ngayBatDau || !ngayKetThuc) {
            swal("Lỗi!", "Vui lòng điền đầy đủ thông tin.", "error");
            return;
        }

        // Check if numeric fields are valid numbers
        if (isNaN(phanTramVoucher) || isNaN(soLuongMuaTT) || isNaN(donGiaMuaTT)) {
            swal("Lỗi!", "Vui lòng nhập giá trị số hợp lệ cho các trường số.", "error");
            return;
        }
        if(phanTramVoucher <= 0||soLuongMuaTT <= 0||donGiaMuaTT <=0){
        ...
        swal({
            ...
            if (willUpdate) {
                fetch('/admin/${nhanVien.maNhanVien}/update-giam-gia-ma-hoa-don', {
                    method: 'POST',
                    body: formData
                })
                    .then(response => {
    ...
    function deleteRow(maVoucher) {
        swal({
            ...
            if (willDelete) {
                fetch('/admin/${nhanVien.maNhanVien}/xoa-ma-giam-gia-hoa-don/' + maVoucher, {
                    method: 'PUT'
                })
                    .then(response => {
                        if (response.ok) {
...
<script>
    ...
    $("#updateTT").click(function (e) {
        ...
        $.ajax({
            ...
            url: "/voucher/update-tt-giam-gia-hoa-don",
            dataType: 'text',
    ...
    $("#updateTTDungHD").click(function (e) {
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json",
            url: "/voucher/update-dung-hoat-dong-toan-bo",
            dataType: 'text',
            success: function (response) {
                console.log(response);
                if (response !== "Success") {
